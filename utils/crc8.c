/** 
 * @file 	crc8.c
 * @author 	Mohit Rathod
 * Created: 22 09 2022, 09:29:41 pm
 * -----
 * Last Modified: 22 09 2022, 09:51:32 pm
 * Modified By  : Mohit Rathod
 * -----
 * MIT License
 * https://opensource.org/licenses/MIT
 * 
 * @brief   CRC-8 implementation for frame checking. CDMA2000
 * 
 */
#include <stddef.h>
#include "crc8.h"

static const uint8_t CRC8_SEED = 0xFF;
static const uint8_t CHECK_VAL = 0xDA;

/**
 * @brief Precomputed CRC8 Table as per the CRC-8/CDMA2000 standards
 */
static const uint8_t _CRCTable[256] = {
	 
};

int validateCRC8()
{
    uint8_t data[] = {0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39};
    return (CHECK_VAL == computeFCS(data, 9)) ? 0 : -1;
}

int computeFCS(const uint8_t *pframe, int len)
{
    return  computeCRC(CRC8_SEED, pframe, len);
}

int computeCRC(const uint8_t crc0, const uint8_t *pbuf, int len)
{
    int ret = -1;
    uint8_t crc = crc0;
    if (pbuf != NULL) {
        while (len--) {
            crc = (uint8_t)_CRCTable[(*pbuf++)^crc];
        }
        ret = crc;
    }
    return ret;
}